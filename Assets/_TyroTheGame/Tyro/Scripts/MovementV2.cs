...
public class MovementV2 : MonoBehaviour
{
    ...
    public void Move()
    {
        ...
        //Preserve the Y 
        lastMoveDirY = moveDirection.y;
        moveDirection = playerInput.x * camRight + playerInput.z * camForward;

        // Adding momentum when the player is in the air, not moving and not dashing

        if (!controller.isGrounded && playerInputRaw.magnitude < 0.2 && !isDashing)
        {
            moveDirection.x += lastMoveDirection.x * momentumVelocity;
            moveDirection.z += lastMoveDirection.z * momentumVelocity;
        ...
        {
            Rotate();
            if (lastPlayerInput.magnitude > 0.1) momentum = lastMoveDirection;
        }
    ...
    public void Dash()
    {
        ...
        {
            // Si no se está moviendo, no puede hacer el dash
            if (canDash)
            {
                isDashing = true;
                canDash = false;
